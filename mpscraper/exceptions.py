...
class ProxyError(Exception):
    """ProxyError: прокси не работает, нужно попробовать следующий."""

    def __init__(self, proxy: str | None, msg: str | None = None):
        self.proxy = proxy
        self.msg = msg

    def __str__(self):
        return f"Прокси {self.proxy} не работает: {self.msg}"


class CategoryStale(ListingsError):
    """Категория устарела: встретили объявление старше max_age_hours. Переходим к следующей категории."""

    def __str__(self):
        return "Category stale (listing older than max_age_hours)"
